...
for (var i = 0; i < parts.length;) {
  parts[i++] = Math.random() * w;
  parts[i++] = Math.random() * h;
  parts[i++] = Math.random() * 2 - 1; // Випадкова швидкість по x
  parts[i++] = Math.random() * 2 - 1; // Випадкова швидкість по y
}
...
c.onmousemove = function (e) {
  var mx = (e.clientX - c.offsetLeft) * srat,
    my = (e.clientY - c.offsetTop) * srat;
  if (!down || mx == start.x && my == start.y) return;
  var sw =
    (floor(mx / s) +
      floor(my / s) * floor(w / s)) * 2;
...
var go = function () {
  var x, y, vx, vy, vd = 0.95,
    ...
    sr = 0.004,
    w1 = w - 1;
  $.fillStyle = 'hsla(205, 75%, 5%, .6)';
  $.globalCompositeOperation = 'source-over';
  $.fillRect(0, 0, w, h);
  $.fillStyle = 'hsla(205, 85%, 35%, 1)';
  $.globalCompositeOperation = 'lighter';
  for (var i = 0, l = parts.length; i < l; i += 4) {
    x = parts[i];
    y = parts[i + 1];
